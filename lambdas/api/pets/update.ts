...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    if (!authCtx) return e(401, 'No autorizado');

    const petId = event.pathParameters?.petId;
    if (!petId) return e(400, 'Falta petId');
    ...
    if (!existing.Item) return e(404, 'Mascota no encontrada');
    ...
    if (!isOwner && !isPrivileged) return e(403, 'Sin permiso para editar esta mascota');
    ...
    if (setExpressions.length === 0) {
      return e(400, 'No hay campos válidos para actualizar');
    }
